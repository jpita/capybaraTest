  ...
  load () {
    ...
    this.basketService.find(parseInt(sessionStorage.getItem('bid'), 10)).subscribe({
      next: (basket) => {
        ...
        this.itemTotal = products.reduce((itemTotal, product) => itemTotal + (product.price ?? 0) * (product.BasketItem?.quantity ?? 0), 0)
        this.bonus = products.reduce((bonusPoints, product) => bonusPoints + Math.round((product.price ?? 0) / 10) * (product.BasketItem?.quantity ?? 0), 0)
